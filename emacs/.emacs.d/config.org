...
Make use-package compute stats.
...
#+BEGIN_SRC elisp
  (add-hook 'after-init-hook (lambda () (global-set-key (kbd "C-x p s") #'eat-project)))
#+END_SRC

I want LSP using Eglot.
...
#+BEGIN_SRC elisp
  (add-hook 'after-init-hook (lambda () (set-face-attribute 'default nil :font "JetBrainsMono Nerd Font" :height 140)))
#+END_SRC
...
I want to only load catppuccin on startup.

#+BEGIN_SRC elisp
  (add-hook 'after-init-hook (lambda () (load-theme 'catppuccin :no-confirm)))
#+END_SRC
